...
function _until(fn_solver) {
    return async function fwrapper() {
        ...
        function solve_me() {
            return fn_solver.apply(window, argv)
        }
...
const vm = {
    ...
    vt: {
        ...
        sixel: function (data) {
            vm.vt.xterm.write(`[sixel:${data && data.length}]\r\n`)
        },
        nodup: 1
    },
...
//TODO: replace by real download progress on .data and wasm instanciation stats.
                    setTimeout(() => {
                        progressElement.value = 125
                    }, 2000)
                    setTimeout(() => {
                        progressElement.value = 150
                    }, 5000)
                    setTimeout(() => {
                        progressElement.value = 190
                    }, 7000)
                }
            }
...
//console.log("MM.prepare", trackid, transport, type)
    ...
    if (audio) {
        ...
        track.set_volume = (v) => {
            track.media.volume = 0.0 + v
        }
        track.get_volume = () => {
            return track.media.volume
        }
        track.stop = () => {
            track.media.pause()
        }
